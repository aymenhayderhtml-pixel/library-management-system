...
function isLikelyAssetMediaType(mediaType) {
    if (!mediaType)
        return false;
...
export async function classifyUrl({ url, fetchImpl, timeoutMs, }) {
    ...
    const tryDetectFromRange = async () => {
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), timeoutMs);
        try {
...
export function buildAssetPromptMessages({ promptText, attachment, }) {
    if (attachment.kind !== "image") {
        throw new Error(`Internal error: tried to build model messages for non-image attachment (${attachment.mediaType}).`);
    }
    return [
