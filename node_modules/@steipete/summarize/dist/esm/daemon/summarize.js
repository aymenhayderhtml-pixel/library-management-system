...
function buildDaemonMetrics({ elapsedMs, summaryFromCache, label, modelLabel, report, costUsd, compactExtraParts, detailedExtraParts, }) {
...
function buildInputSummaryForExtracted(extracted) {
    const isYouTube = extracted.siteName === "YouTube" || isYouTubeUrl(extracted.url);
    const transcriptChars = typeof extracted.transcriptCharacters === "number" && extracted.transcriptCharacters > 0
        ? extracted.transcriptCharacters
        : null;
    const hasTranscript = transcriptChars != null;
    const transcriptWords = hasTranscript && transcriptChars != null
    ...
    const exactDurationSeconds = typeof extracted.mediaDurationSeconds === "number" && extracted.mediaDurationSeconds > 0
        ...
        : null;
    const estimatedDurationSeconds = transcriptWords != null && transcriptWords > 0
...
export async function streamSummaryForVisiblePage({ env, fetchImpl, input, modelOverride, promptOverride, lengthRaw, languageRaw, format, sink, cache, mediaCache, overrides, }) {
...
export async function streamSummaryForUrl({ env, fetchImpl, input, modelOverride, promptOverride, lengthRaw, languageRaw, format, sink, cache, mediaCache, overrides, slides, hooks, }) {
    const startedAt = Date.now();
    let usedModel = null;
    let summaryFromCache = false;
    const extractedRef = { value: null };
    const writeStatus = typeof sink.writeStatus === "function" ? sink.writeStatus : null;
    const ctx = createDaemonUrlFlowContext({
        env,
        fetchImpl,
        ...
        lengthRaw,
        languageRaw,
        maxExtractCharacters: input.maxCharacters && input.maxCharacters > 0 ? input.maxCharacters : null,
        format,
        overrides,
        slides,
        hooks: {
...
export async function extractContentForUrl({ env, fetchImpl, input, cache, mediaCache, overrides, format, slides, hooks, }) {
    const extractedRef = { value: null };
    const slidesRef = { value: null };
    const ctx = createDaemonUrlFlowContext({
        env,
        fetchImpl,
        cache,
