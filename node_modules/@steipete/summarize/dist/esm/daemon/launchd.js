...
export function resolveLaunchctlTargetUid({ uid, sudoUid, } = {}) {
    const currentUid = typeof uid === "number" ? uid : typeof process.getuid === "function" ? process.getuid() : null;
    if (currentUid === 0) {
    ...
    if (fallbackSudo !== null)
        return fallbackSudo;
    return 501;
}
export function resolveLaunchctlDomains({ uid, sudoUid, } = {}) {
...
export async function restartLaunchAgent({ stdout, }) {
    ...
    throw new Error(`launchctl kickstart failed: ${lastResult?.stderr || lastResult?.stdout || "unknown error"}`.trim());
}
