...
function isWindowsShimPath(filePath) {
    return /\.(cmd|bat|ps1)$/i.test(filePath);
}
export async function resolveCliEntrypointCandidatesFromWindowsShim(shimPath) {
    if (!isWindowsShimPath(shimPath))
        return [];
    ...
    try {
        ...
        const tokenMatch = contents.match(/(?:%~dp0|%dp0%|\$basedir)[^"'\\r\\n]*node_modules[\\/]+@steipete[\\/]+summarize[\\/]+dist[\\/]+cli\.(?:cjs|js)/i);
        const matches = Array.from(contents.matchAll(/["']?([^"'\\r\\n]*node_modules[\\/]+@steipete[\\/]+summarize[\\/]+dist[\\/]+cli\.(?:cjs|js))["']?/gi));
        const preferred = tokenMatch?.[0] ??
...
export async function resolveCliEntrypointPathForService() {
...
//# sourceMappingURL=cli-entrypoint.js.map