...
function resolveHomeDir(env) {
    ...
    if (!home)
        throw new Error("Missing HOME (required for daemon config)");
    return home;
}
export function resolveDaemonConfigPath(env) {
    const home = resolveHomeDir(env);
    return path.join(home, DAEMON_CONFIG_DIR, DAEMON_CONFIG_FILENAME);
}
export function normalizeDaemonToken(raw) {
    const token = raw.trim();
    if (!token)
        throw new Error("Missing token");
    if (token.length < 16)
        throw new Error("Token too short (expected >= 16 chars)");
    return token;
}
export function normalizeDaemonPort(raw) {
...
export async function readDaemonConfig({ env, }) {
    ...
    const envRaw = obj.env && typeof obj.env === "object" ? obj.env : {};
    const envSnapshot = {};
    for (const [k, v] of Object.entries(envRaw)) {
        ...
        if (!trimmed)
            continue;
        envSnapshot[k] = trimmed;
    }
    const installedAt = typeof obj.installedAt === "string" ? obj.installedAt : new Date().toISOString();
    return { version: 1, token, port, env: envSnapshot, installedAt };
}
export async function writeDaemonConfig({ env, config, }) {
    const configPath = resolveDaemonConfigPath(env);
