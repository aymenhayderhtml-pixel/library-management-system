...
export function parseExtractFormat(raw) {
    const normalized = raw.trim().toLowerCase();
...
export function parsePreprocessMode(raw) {
    const normalized = raw.trim().toLowerCase();
    if (normalized === "off" || normalized === "auto" || normalized === "always") {
        return normalized;
    }
...
export function parseVideoMode(raw) {
    const normalized = raw.trim().toLowerCase();
...
export function parseLengthArg(raw) {
    const normalized = raw.trim().toLowerCase();
    const shorthand = { s: "short", m: "medium", l: "long" };
    if (normalized in shorthand) {
        return { kind: "preset", preset: shorthand[normalized] };
    }
...
export function parseMaxExtractCharactersArg(raw) {
    ...
    const maxCharacters = Math.floor(numeric * multiplier);
    if (maxCharacters < MIN_LENGTH_CHARS) {
        throw new Error(`Unsupported --max-extract-characters: ${raw} (minimum ${MIN_LENGTH_CHARS} chars)`);
    }
...
export function parseRetriesArg(raw) {
    const normalized = raw.trim();
    if (!normalized) {
