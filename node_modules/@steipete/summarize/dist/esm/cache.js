...
function normalizeTranscriptSource(value) {
    if (typeof value !== "string")
        return null;
    return TRANSCRIPT_SOURCES.includes(value)
        ? value
        : null;
}
...
function cleanupSlidesPayload(raw) {
    let payload;
    try {
...
export function buildSlidesCacheKey({ url, settings, }) {
    return hashJson({
        url,
        settings: {
            ocr: settings.ocr,
...
export function buildTranscriptCacheKey({ url, namespace, formatVersion, fileMtime, }) {
    return hashJson({
        url,
        namespace,
