...
const DEFAULT_SOURCES = ["chrome", "safari", "firefox"];
...
function parseCookieSourceList(value, warnings) {
    const tokens = value
        .split(/[,\s]+/)
...
function resolveEnvCookieSource(env, warnings) {
...
function resolveEnvProfile(env, keys) {
...
function formatBrowserSourceLabel(browser, profile) {
    if (browser === "chrome")
...
function buildCookiesFromBrowserSpec(browser, profile) {
...
function resolveChromeCookiesDb(profile, platform, homeDir, env) {
...
function resolveFirefoxCookiesDb(profile, platform, homeDir, env) {
...
function resolveSafariCookiesFile(platform, homeDir) {
    if (platform !== "darwin")
        return null;
    const candidates = [
        path.join(homeDir, "Library", "Cookies", "Cookies.binarycookies"),
        path.join(homeDir, "Library", "Containers", "com.apple.Safari", "Data", "Library", "Cookies", "Cookies.binarycookies"),
    ];
    for (const candidate of candidates) {
        if (existsSync(candidate))
            return candidate;
...
function hasCookiesStore(browser, profile, platform, homeDir, env) {
    if (browser === "safari") {
...
export async function resolveTwitterCookies({ env, cookieSource, chromeProfile, firefoxProfile, platform, homeDir, }) {
    const warnings = [];
    const cookies = {
    ...
    const sourcesToTry = Array.isArray(effectiveCookieSource)
