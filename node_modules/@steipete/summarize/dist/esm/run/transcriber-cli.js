...
const ONNX_MODELS = ["parakeet", "canary"];
const parseModel = (value) => {
    if (!value)
        return "parakeet";
    const normalized = value.trim().toLowerCase();
    if (normalized === "parakeet" || normalized === "canary")
        return normalized;
    throw new Error(`Unsupported --model: ${value}`);
};
const readArgValue = (normalizedArgv, name) => {
...
const isBinaryAvailable = async (binary, env) => {
...
const resolveWhisperCppModelPath = (env) => {
...
const renderOnnxEnvExample = (model) => {
    if (model === "canary") {
        return [
...
export async function handleTranscriberCliRequest({ normalizedArgv, envForRun, stdout, }) {
    if (normalizedArgv[0]?.toLowerCase() !== "transcriber")
        return false;
    const subcommand = normalizedArgv[1]?.toLowerCase() ?? "help";
    const help = subcommand === "help" || normalizedArgv.includes("--help") || normalizedArgv.includes("-h");
    if (help) {
    ...
    const label = (text) => theme.label(text);
    const value = (text) => theme.value(text);
    ...
    stdout.write(`${label("Auto order:")} ${value("ONNX (parakeet then canary) -> whisper.cpp -> OpenAI/FAL")}\n`);
    stdout.write("\n");
    for (const entry of onnxStatus) {
        stdout.write(`${label(`ONNX ${entry.model}:`)} ${value(entry.configured ? "configured" : "not configured")} ${dim(`(${entry.envKey})`)}\n`);
    }
    stdout.write(`${label("ONNX cache:")} ${value(onnxCacheDir)}\n`);
    stdout.write(`${label(`ONNX ${model} artifacts:`)} ${value(modelReady ? "present" : "missing")}\n`);
    stdout.write("\n");
    stdout.write(`${label("whisper.cpp:")} ${value(whisperCliReady ? "binary ok" : "binary missing")} ${dim(`(${whisperBinary})`)}\n`);
    stdout.write(`${label("whisper.cpp model:")} ${value(whisperModelReady ? "present" : "missing")} ${dim(`(${whisperModelPath})`)}\n`);
    stdout.write("\n");
    if (!onnxStatus.some((entry) => entry.configured)) {
        stdout.write(`${heading("To enable ONNX locally:")}\n`);
        stdout.write(`  ${dim("Install sherpa-onnx from upstream binaries or build; Homebrew may not have a formula.")}\n`);
        for (const line of renderOnnxEnvExample(model)) {
            stdout.write(`  ${line}\n`);
        }
        stdout.write("  # placeholders: {input}, {model}, {vocab}, {model_dir} (see docs/nvidia-onnx-transcription.md)\n");
    ...
    stdout.write(`  ${value('SUMMARIZE_TRANSCRIBER=auto summarize "https://..." --extract --format md')}\n`);
    return true;
}
//# sourceMappingURL=transcriber-cli.js.map