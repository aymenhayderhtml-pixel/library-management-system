import { formatCompactCount, formatDurationSecondsSmart, formatElapsedMs, formatMinutesSmart, } from "../tty/format.js";
...
export function formatModelLabelForDisplay(model) {
...
function inferMediaKindLabelForFinishLine(extracted) {
    ...
    const hasTranscript = typeof extracted.transcriptCharacters === "number" && extracted.transcriptCharacters > 0;
    if (!hasTranscript)
        return null;
    return "audio";
}
function buildCompactTranscriptPart(extracted) {
    ...
    const exactDurationSeconds = typeof extracted.mediaDurationSeconds === "number" && extracted.mediaDurationSeconds > 0
        ? extracted.mediaDurationSeconds
        : null;
    const duration = exactDurationSeconds != null
...
function buildDetailedLengthPartsForExtracted(extracted) {
    ...
    if (!isYouTube && !extracted.transcriptCharacters)
        return parts;
    const transcriptChars = extracted.transcriptCharacters;
    const shouldOmitInput = typeof transcriptChars === "number" &&
        transcriptChars > 0 &&
    ...
    if (typeof extracted.transcriptCharacters === "number" && extracted.transcriptCharacters > 0) {
        ...
        const durationPart = typeof extracted.mediaDurationSeconds === "number" && extracted.mediaDurationSeconds > 0
    ...
    if (hasTranscript && extracted.transcriptSource) {
        const providerSuffix = extracted.transcriptSource === "whisper" &&
        ...
        const cachePart = typeof cacheStatus === "string" && cacheStatus !== "unknown" ? cacheStatus : null;
        const txParts = [`tx=${extracted.transcriptSource}${providerSuffix}`];
        if (cachePart)
...
export function buildLengthPartsForFinishLine(extracted, detailed) {
...
export function writeFinishLine({ stderr, elapsedMs, elapsedLabel, label, model, report, costUsd, detailed, extraParts, color, env, }) {
...
export function buildFinishLineText({ elapsedMs, elapsedLabel, label, model, report, costUsd, detailed, extraParts, }) {
...
export function buildFinishLineVariants({ elapsedMs, elapsedLabel, label, model, report, costUsd, compactExtraParts, detailedExtraParts, }) {
...
export function formatFinishLineText(model, detailed) {
    const line = model.lineParts.join(" · ");
    if (!detailed || model.detailParts.length === 0)
        return { line, details: null };
    return { line, details: model.detailParts.join(" | ") };
}
export function buildFinishLineModel({ elapsedMs, elapsedLabel, label, model, report, costUsd, extraParts, }) {
    const resolvedElapsedLabel = typeof elapsedLabel === "string" && elapsedLabel.trim().length > 0
        ? elapsedLabel
    ...
    const tokensPart = hasAnyTokens
        ? `↑${promptTokens != null ? formatCompactCount(promptTokens) : "unknown"} ↓${completionTokens != null ? formatCompactCount(completionTokens) : "unknown"} Δ${totalTokens != null ? formatCompactCount(totalTokens) : "unknown"}`
        : null;
    ...
    const filteredExtraParts = compactTranscriptLabel && extraParts
    ...
    const lineParts = summaryParts.filter((part) => typeof part === "string");
    const totalCalls = report.llm.reduce((sum, row) => sum + row.calls, 0);
    const lenParts = filteredExtraParts?.filter((part) => part.startsWith("input=") || part.startsWith("transcript=")) ?? [];
    const miscParts = filteredExtraParts?.filter((part) => !part.startsWith("input=") && !part.startsWith("transcript=")) ?? [];
    const line2Segments = [];
...
export function buildSummaryFinishLabel(args) {
    ...
    const transcriptProvided = Boolean(args.extracted.diagnostics.transcript?.textProvided);
    const words = typeof args.extracted.wordCount === "number" && Number.isFinite(args.extracted.wordCount)
...
//# sourceMappingURL=finish-line.js.map