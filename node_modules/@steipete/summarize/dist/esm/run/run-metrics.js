...
export function createRunMetrics({ env, fetchImpl, maxOutputTokensArg, }) {
    const llmCalls = [];
    ...
    const setTranscriptionCost = (costUsd, label) => {
    ...
    const estimateCostUsd = async () => {
        ...
        if (calls.length === 0) {
            if (explicitCosts.length > 0 || hasExtra)
                return explicitTotal + extraTotal;
            return null;
        }
        const catalog = await getLiteLlmCatalog();
        if (!catalog) {
        ...
        if (catalogTotal === null && explicitCosts.length === 0 && !hasExtra)
            return null;
...
//# sourceMappingURL=run-metrics.js.map