...
async function readStreamToBuffer(stream, maxBytes) {
    ...
    for await (const chunk of stream) {
        ...
        if (totalSize > maxBytes) {
            throw new Error(`Stdin content exceeds maximum size of ${(maxBytes / 1024 / 1024).toFixed(1)}MB`);
        }
        chunks.push(buffer);
    }
...
async function resolveStdinExtensionAndKind(bytes) {
...
export async function createTempFileFromStdin({ stream, maxBytes = DEFAULT_STDIN_MAX_BYTES, }) {
...
//# sourceMappingURL=stdin-temp-file.js.map