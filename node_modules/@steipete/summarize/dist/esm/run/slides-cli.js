...
import { extractSlidesForSource, resolveSlideSettings, resolveSlideSourceFromUrl, } from "../slides/index.js";
...
import { renderSlidesInline } from "./slides-render.js";
...
function parseRenderMode(raw) {
    const value = typeof raw === "string" ? raw.trim().toLowerCase() : "";
    if (!value || value === "none")
        return "none";
    if (value === "auto" || value === "kitty" || value === "iterm")
        return value;
    throw new Error(`Unsupported --render: ${String(raw)}`);
}
export async function handleSlidesCliRequest({ normalizedArgv, envForRun, stdout, stderr, }) {
    if (normalizedArgv[0]?.toLowerCase() !== "slides")
        return false;
    ...
    const slidesSettings = resolveSlideSettings({
        ...
        slidesSceneThresholdExplicit: normalizedArgv.some((arg) => arg === "--slides-scene-threshold" || arg.startsWith("--slides-scene-threshold=")),
    ...
    const renderStatus = (label, detail = "…") => `${theme.label(label)}${theme.dim(detail)}`;
    ...
    if (renderMode !== "none") {
        ...
        await renderSlidesInline({
            ...
            env: envForRun,
            stdout,
            labelForSlide: (slide) => `Slide ${slide.index} · ${formatTimestamp(slide.timestamp)} (${slide.imagePath})`,
        });
        return true;
    }
...
//# sourceMappingURL=slides-cli.js.map