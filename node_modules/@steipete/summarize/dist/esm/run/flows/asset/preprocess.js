...
import { getFileBytesFromAttachment, getTextContentFromAttachment, MAX_DOCUMENT_BYTES_DEFAULT, shouldMarkitdownConvertMediaType, supportsNativeFileAttachment, } from "../../attachments.js";
import { MAX_TEXT_BYTES_DEFAULT } from "../../constants.js";
import { hasUvxCli } from "../../env.js";
import { withUvxTip } from "../../tips.js";
export function resolveDocumentHandling({ attachment, textContent, fileBytes, preprocessMode, fixedModelSpec, openaiApiKey, openrouterApiKey, openaiBaseUrl, }) {
    ...
    const canAttachDocument = (() => {
        ...
        if (fixedModelSpec?.transport !== "native")
            return false;
        if (!supportsNativeFileAttachment({
    ...
    if (canAttachDocument && fileBytes.byteLength > MAX_DOCUMENT_BYTES_DEFAULT) {
        if (preprocessMode === "off") {
            return {
                mode: "error",
                error: new Error(`PDF is too large to attach (${formatBytes(fileBytes.byteLength)}). Max is ${formatBytes(MAX_DOCUMENT_BYTES_DEFAULT)}. Enable preprocessing or use a smaller file.`),
            };
        }
    ...
    if (preprocessMode === "off") {
        return {
            mode: "error",
            error: new Error(`This build does not support attaching binary files (${attachment.mediaType}). Enable preprocessing (e.g. --preprocess auto) and install uvx/markitdown.`),
        };
    }
    return { mode: "preprocess" };
}
export async function prepareAssetPrompt({ ctx, attachment, }) {
    ...
    const buildInlinePromptText = ({ content, contentMediaType, originalMediaType, }) => {
    ...
    const documentHandling = attachment.kind === "file"
    ...
    if (attachment.kind === "file" && !textContent && documentHandling.mode === "preprocess") {
        ...
        if (!shouldMarkitdownConvertMediaType(attachment.mediaType)) {
            throw new Error(`Unsupported file type: ${attachment.filename ?? "file"} (${attachment.mediaType})\n` +
