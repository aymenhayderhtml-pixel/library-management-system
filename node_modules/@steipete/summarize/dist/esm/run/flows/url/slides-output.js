...
import { buildTimestampUrl, formatOsc8Link, formatTimestamp, splitSlideTitleFromText, } from "./slides-text.js";
function createSlideOutputState(initialSlides) {
    ...
    let done = false;
    const updateSlideEntry = (slide) => {
        const existing = slidesByIndex.get(slide.index);
        const next = {
            index: slide.index,
            timestamp: Number.isFinite(slide.timestamp) && slide.timestamp >= 0
                ? slide.timestamp
    ...
    if (initialSlides)
        updateFromSlides(initialSlides);
    const markDone = () => {
        if (done)
            return;
        done = true;
    ...
    return {
        ...
        waitForSlide,
        markDone,
        getSlides: () => order.map((index) => slidesByIndex.get(index)).filter(Boolean),
        getSlide: (index) => slidesByIndex.get(index) ?? null,
...
export function createSlidesTerminalOutput({ io, flags, extracted, slides, enabled, outputMode, clearProgressForStdout, restoreProgressAfterStdout, onProgressText, }) {
    if (!enabled)
        return null;
    ...
    const noteInlineUnsupported = (nextSlides) => {
        if (!inlineNoticeEnabled || inlineNoticeShown)
            return;
        if (!nextSlides.slidesDir)
            return;
        ...
        io.stderr.write(`Slides saved to ${nextSlides.slidesDir}. Inline images unavailable (${reason}).\n`);
    ...
    const onSlidesExtracted = (nextSlides) => {
    ...
    const onSlidesDone = (_result) => {
        state.markDone();
    };
    let renderedCount = 0;
    const renderSlide = async (index, title) => {
        if (index <= 0)
            return;
        ...
        const timestampLabel = typeof timestamp === "number" && Number.isFinite(timestamp)
            ...
            : null;
        const timestampUrl = typeof timestamp === "number" && Number.isFinite(timestamp)
        ...
        const titleMax = 90;
        const shortTitle = cleanTitle.length > titleMax
    ...
    const streamHandler = createSlidesSummaryStreamHandler({
        stdout: io.stdout,
        env: io.env,
        ...
        getSlideMeta: (index) => {
            const total = state.getOrder().length || (slides?.slides.length ?? 0);
            const slide = state.getSlide(index);
            const timestamp = typeof slide?.timestamp === "number" && Number.isFinite(slide.timestamp)
                ? slide.timestamp
                : null;
            return { total, timestamp };
        },
        debugWrite: io.envForRun.SUMMARIZE_DEBUG_SLIDE_MARKERS &&
...
export function createSlidesSummaryStreamHandler({ stdout, env, envForRun, plain, outputMode, clearProgressForStdout, restoreProgressAfterStdout, renderSlide, getSlideIndexOrder, getSlideMeta, debugWrite, }) {
    ...
    const handleMarkdownChunk = (nextVisible, prevVisible) => {
        if (!streamer)
            return;
    ...
    const pushVisibleLines = (segment) => {
        if (!segment)
            return;
    ...
    const flushPendingSlide = async (force) => {
        if (!pendingSlide)
            return;
        ...
        if (body.trim())
            pushVisibleLines(body);
    };
    const appendVisible = async (segment) => {
        if (!segment)
            return;
        const sanitized = segment.replace(slideStripRegex, "");
        if (!sanitized)
    ...
    const flushBuffered = async ({ final }) => {
        while (buffered.length > 0) {
            ...
            const fallbackMatch = fallbackStart >= 0 && fallbackEnd > fallbackStart
                ? { start: fallbackStart, end: fallbackEnd }
                : null;
            const nextMatch = tagMatch && labelMatch
                ? (tagMatch.index ?? 0) <= (labelMatch.index ?? 0)
                    ? { kind: "tag", match: tagMatch }
                    : { kind: "label", match: labelMatch }
                : tagMatch
                    ? { kind: "tag", match: tagMatch }
                    : labelMatch
                        ? { kind: "label", match: labelMatch }
                        : fallbackMatch
                            ? { kind: "fallback", match: fallbackMatch }
                            : null;
            if (!nextMatch) {
                if (final) {
            ...
            const matchIndex = nextMatch.kind === "fallback" ? nextMatch.match.start : (nextMatch.match.index ?? 0);
            const matchLength = nextMatch.kind === "fallback"
            ...
            if (nextMatch.kind === "fallback") {
                ...
                const rawIndex = nextMatch.kind === "tag"
                    ? nextMatch.match[1]
            ...
            if (Number.isFinite(index) && (index ?? 0) > 0) {
                if (getSlideMeta) {
                    pendingSlide = { index: index, buffer: "" };
                }
                else {
                    await renderSlideBlock(index, null);
                }
...
//# sourceMappingURL=slides-output.js.map