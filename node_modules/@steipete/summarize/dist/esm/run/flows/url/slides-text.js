...
const SLIDE_TEXT_BUDGET_BY_PRESET = {
    short: 120,
    medium: 200,
    long: 320,
    xl: 480,
...
const SLIDE_WINDOW_SECONDS_BY_PRESET = {
    short: 30,
    medium: 60,
    long: 90,
    xl: 120,
...
const SLIDE_LABEL_PATTERN = /^(?:\[)?slide\s+(\d+)(?:\s*(?:\/|of)\s*\d+)?(?:\])?(?:\s*[\u00b7:-]\s*.*)?$/i;
const TITLE_ONLY_MAX_CHARS = 80;
const clampNumber = (value, min, max) => Math.min(max, Math.max(min, value));
const collapseLineWhitespace = (value) => value.replace(/\s+/g, " ").trim();
const deriveHeadlineFromBody = (body) => {
...
const isTitleOnlySlideText = (value) => {
    ...
    if (lines.length !== 1)
        return false;
    if (trimmed.length > TITLE_ONLY_MAX_CHARS)
        return false;
    if (/[.!?]/.test(trimmed))
        return false;
    return true;
};
const stripSlideTitleList = (markdown) => {
    if (!markdown.trim())
        return markdown;
...
export const splitSlideTitleFromText = ({ text, }) => {
    ...
    if (cleaned.length === 0)
        return { title: null, body: "" };
    const filtered = cleaned.filter((line) => !SLIDE_LABEL_PATTERN.test(line) && !SLIDE_TAG_PATTERN.test(line));
...
export const ensureSlideTitleLine = ({ text, slide, total, }) => {
    void slide;
    void total;
    return text.trim();
};
export function findSlidesSectionStart(markdown) {
    if (!markdown)
        return null;
    ...
    const indexes = [heading?.index, tag?.index, label?.index].filter((idx) => idx != null);
    if (indexes.length === 0)
        return null;
    return Math.min(...indexes);
}
export function splitSummaryFromSlides(markdown) {
    const start = findSlidesSectionStart(markdown);
    if (start == null)
        return { summary: markdown.trim(), slidesSection: null };
    const summary = markdown.slice(0, start).trim();
    const slidesSection = markdown.slice(start);
    return { summary, slidesSection };
}
export function parseSlideSummariesFromMarkdown(markdown) {
    ...
    const hasFutureMarker = (start) => lines.slice(start).some((line) => {
        const trimmed = line.trim();
...
function parseInlineSlideSummaries(markdown) {
    const result = new Map();
...
export function extractSlideMarkers(markdown) {
    if (!markdown.trim())
        return [];
    const indexes = [];
...
export function normalizeSummarySlideHeadings(markdown) {
    if (!markdown.trim())
        return markdown;
    if (!/\[slide:\d+\]/i.test(markdown))
        return markdown;
    const deleteMarker = "__SUMMARIZE_DELETE__";
    const lines = markdown.split("\n");
    for (let i = 0; i < lines.length; i += 1) {
        const line = lines[i] ?? "";
        if (!SLIDE_TAG_PATTERN.test(line.trim()))
            continue;
...
function pickIntroParagraph(markdown) {
    const paragraphs = splitMarkdownParagraphs(markdown);
    if (paragraphs.length === 0)
        return "";
    const firstNonHeading = paragraphs.find((paragraph) => !/^#{1,6}\s+\S/.test(paragraph.trim())) ?? paragraphs[0];
    if (!firstNonHeading)
        return "";
    const sentences = firstNonHeading.match(/[^.!?]+[.!?]+|[^.!?]+$/g) ?? [firstNonHeading];
    if (sentences.length <= 3)
        return firstNonHeading.trim();
    return sentences.slice(0, 3).join(" ").trim();
}
export function buildSlideTextFallback({ slides, transcriptTimedText, lengthArg, }) {
    const map = new Map();
    if (!transcriptTimedText || !transcriptTimedText.trim())
        return map;
    if (slides.length === 0)
        return map;
    ...
    for (let i = 0; i < ordered.length; i += 1) {
        ...
        if (text)
            map.set(slide.index, text);
    }
    return map;
}
export function coerceSummaryWithSlides({ markdown, slides, transcriptTimedText, lengthArg, }) {
    ...
    const titleOnlySlideSummaries = slideSummaries.size > 0 &&
    ...
    const remaining = introIndex >= 0 ? paragraphs.filter((_, index) => index !== introIndex) : paragraphs.slice(1);
    const parts = [];
    if (introParagraph)
        parts.push(introParagraph.trim());
...
export function interleaveSlidesIntoTranscript({ transcriptTimedText, slides, }) {
    if (!transcriptTimedText.trim() || slides.length === 0)
...
export function parseTranscriptTimedText(input) {
    if (!input)
        return [];
    const segments = [];
...
function truncateSlideText(value, limit) {
...
export function resolveSlideTextBudget({ lengthArg, slideCount, }) {
...
export function resolveSlideWindowSeconds({ lengthArg, }) {
    if (lengthArg.kind === "preset") {
...
export function getTranscriptTextForSlide({ slide, nextSlide, segments, budget, windowSeconds, }) {
    if (!Number.isFinite(slide.timestamp))
        return "";
    if (segments.length === 0)
        return "";
    ...
    for (const segment of segments) {
        if (segment.startSeconds < lower)
            continue;
        if (segment.startSeconds > upper)
            break;
...
export function formatOsc8Link(label, url, enabled) {
    if (!enabled || !url)
        return label;
    const osc = "\u001b]8;;";
    const st = "\u001b\\";
...
export function buildTimestampUrl(sourceUrl, seconds) {
    if (!sourceUrl)
        return null;
    let url;
    try {
...
//# sourceMappingURL=slides-text.js.map