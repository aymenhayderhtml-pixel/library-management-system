...
let cachedOpenRouterIndex = null;
let cachedOpenRouterIndexReady = false;
function buildOpenRouterModelIndex(modelIds) {
    const byId = new Map();
...
function resolveOpenRouterModelIdForNative({ nativeModelId, index, }) {
    ...
    if (slash === -1 || slash === canonicalLower.length - 1)
        return null;
    ...
    if (!normalizedMatches || normalizedMatches.size !== 1)
        return null;
...
const DEFAULT_AUTO_CLI_ORDER = ["claude", "gemini", "codex", "agent"];
function dedupeCliProviderOrder(order) {
    const out = [];
...
export function resolveCliAutoFallbackConfig(config) {
    const raw = (config?.cli?.autoFallback ??
        config?.cli?.magicAuto ??
        null);
    const order = Array.isArray(raw?.order) && raw.order.length > 0 ? raw.order : DEFAULT_AUTO_CLI_ORDER;
    return {
...
function hasAnyApiKeysConfigured(env) {
    const has = (value) => typeof value === "string" && value.trim().length > 0;
    return Boolean(has(env.OPENAI_API_KEY) ||
...
function prioritizeCliProvider(providers, preferred) {
    if (!preferred)
        return providers;
...
function isCliProviderEnabled(provider, config) {
    const cli = config?.cli;
    if (!Array.isArray(cli?.enabled) || cli.enabled.length === 0)
        return false;
    return cli.enabled.includes(provider);
}
function isCandidateOpenRouter(modelId) {
    return modelId.trim().toLowerCase().startsWith("openrouter/");
}
function isCandidateCli(modelId) {
    return modelId.trim().toLowerCase().startsWith("cli/");
}
function parseCliCandidate(modelId) {
...
function requiredEnvForCandidate(modelId) {
    if (isCandidateCli(modelId)) {
    ...
    if (isCandidateOpenRouter(modelId))
...
export function envHasKey(env, requiredEnv) {
...
function tokenMatchesBand({ promptTokens, band, }) {
...
function resolveRuleCandidates({ kind, promptTokens, config, }) {
    const rules = (() => {
...
function prependCliCandidates({ candidates, config, env, isImplicitAutoSelection, allowAutoCliFallback, lastSuccessfulCliProvider, }) {
    const cli = config?.cli;
    ...
    const cliCandidates = [];
    const add = (provider, modelOverride) => {
        if (hasExplicitEnabledList && !isCliProviderEnabled(provider, config))
            return;
        const model = modelOverride?.trim() || DEFAULT_CLI_MODELS[provider];
        if (!model)
            return;
        const id = `cli/${provider}/${model}`;
        if (!cliCandidates.includes(id))
            cliCandidates.push(id);
    };
    for (const provider of providerOrder) {
        const modelOverride = provider === "gemini"
            ? cli?.gemini?.model
            : provider === "codex"
                ? cli?.codex?.model
                : provider === "agent"
...
function estimateCostUsd({ pricing, promptTokens, outputTokens, }) {
    if (!pricing)
        return null;
    if (typeof pricing.inputUsdPerToken !== "number" || typeof pricing.outputUsdPerToken !== "number")
        return null;
    const inTok = typeof promptTokens === "number" && Number.isFinite(promptTokens) && promptTokens > 0
        ? promptTokens
        : 0;
    const outTok = typeof outputTokens === "number" && Number.isFinite(outputTokens) && outputTokens > 0
        ? outputTokens
        : 0;
    const cost = inTok * pricing.inputUsdPerToken + outTok * pricing.outputUsdPerToken;
    return Number.isFinite(cost) ? cost : null;
}
function isVideoUnderstandingCapable(modelId) {
    try {
        const parsed = parseGatewayStyleModelId(normalizeGatewayStyleModelId(modelId));
...
export function buildAutoModelAttempts(input) {
    ...
    const shouldResolveOpenRouterIndex = !input.requiresVideoUnderstanding && envHasKey(input.env, "OPENROUTER_API_KEY");
    ...
    for (const modelRawEntry of candidates) {
        ...
        const shouldSkipForVideo = input.requiresVideoUnderstanding &&
            (explicitOpenRouter || explicitCli || !isVideoUnderstandingCapable(modelRaw));
        if (shouldSkipForVideo) {
            continue;
        }
        const addAttempt = (modelId, options) => {
            const required = requiredEnvForCandidate(modelId);
            const hasKey = options.transport === "cli"
                ? Boolean(input.cliAvailability?.[parseCliCandidate(modelId)?.provider ?? "claude"] ?? false)
            ...
            const userModelId = options.transport === "cli"
                ? modelId
                : options.openrouter
                    ? modelId
            ...
            const llmModelId = options.transport === "cli"
                ? null
                : options.openrouter
                    ? `openai/${openrouterModelId}`
                    : normalizeGatewayStyleModelId(modelId);
            const debugParts = [
                `model=${options.transport === "cli"
                    ? userModelId
                    : options.openrouter
        ...
        const canAddOpenRouterFallback = !input.requiresVideoUnderstanding && envHasKey(input.env, "OPENROUTER_API_KEY");
...
//# sourceMappingURL=model-auto.js.map