...
export const DEFAULT_MEDIA_CACHE_VERIFY = "size";
const INDEX_VERSION = 1;
const INDEX_FILENAME = "index.json";
const ensureDir = async (path) => {
    await fs.mkdir(path, { recursive: true });
};
export function resolveMediaCachePath({ env, cachePath, }) {
...
const resolveExtension = (filename, mediaType) => {
...
const removeEntry = async (cacheDir, index, key, entry) => {
...
const normalizeEntry = (cacheDir, entry) => {
    return {
        url: entry.url,
...
export async function createMediaCache({ path, maxBytes, ttlMs, verify = DEFAULT_MEDIA_CACHE_VERIFY, }) {
    const cacheDir = path;
    const indexPath = join(cacheDir, INDEX_FILENAME);
    await ensureDir(cacheDir);
    const pruneExpired = async (index, now) => {
    ...
    const computeSizeBytes = async (entry) => {
        try {
    ...
    const enforceMaxBytes = async (index) => {
    ...
    const put = async ({ url, filePath, mediaType = null, filename = null, }) => {
        if (!shouldCacheUrl(url))
            return null;
        const now = Date.now();
        const index = await readIndex(indexPath);
