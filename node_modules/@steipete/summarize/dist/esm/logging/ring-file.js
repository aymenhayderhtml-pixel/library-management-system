...
const normalizeMaxFiles = (value) => Number.isFinite(value) && value > 0 ? Math.max(1, Math.trunc(value)) : 1;
const normalizeMaxBytes = (value) => Number.isFinite(value) && value > 0 ? Math.max(1, Math.trunc(value)) : 1024;
...
async function rotateFiles(filePath, maxFiles) {
    if (maxFiles <= 1) {
        try {
...
export function createRingFileWriter(options) {
...
//# sourceMappingURL=ring-file.js.map