...
function formatPrettyLine({ metaMarkup, args, errors, }) {
    const parts = [];
    const meta = metaMarkup.trim();
    if (meta)
        parts.push(meta);
    if (args.length > 0) {
        parts.push(args.map((arg) => (typeof arg === "string" ? arg : safeJsonStringify(arg))).join(" "));
    }
...
export function resolveDaemonLoggingConfig({ env, config, }) {
    const logging = config?.logging;
    if (!logging || logging.enabled !== true)
        return null;
    const { logDir } = resolveDaemonLogPaths(env);
    const file = typeof logging.file === "string" && logging.file.trim()
        ? logging.file.trim()
        : path.join(logDir, "daemon.jsonl");
    const maxMb = typeof logging.maxMb === "number" && logging.maxMb > 0 ? logging.maxMb : DEFAULT_LOG_MAX_MB;
    const maxFiles = typeof logging.maxFiles === "number" && logging.maxFiles > 0
...
export function createDaemonLogger({ env, config, }) {
    ...
    const logger = resolved.format === "pretty"
        ? new Logger({
            ...baseSettings,
            type: "pretty",
            overwrite: {
    ...
    const getSubLogger = (name, logObj) => logger.getSubLogger({ name }, logObj);
    return {
        enabled: true,
        config: resolved,
        logger,
