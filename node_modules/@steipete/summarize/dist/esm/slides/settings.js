...
const parsePositiveInt = (raw, label, min = 1) => {
    if (raw == null)
        return null;
...
const parseNumberInRange = (raw, label, { min, max }) => {
    if (raw == null)
        return null;
    const value = typeof raw === "string" ? raw.trim() : raw;
    const numeric = typeof value === "number" ? value : Number(value);
...
export function resolveSlideSettings(input) {
    ...
    const sceneThreshold = parseNumberInRange(input.slidesSceneThreshold, "--slides-scene-threshold", {
        min: 0.1,
        max: 1,
    }) ?? DEFAULT_SCENE_THRESHOLD;
    const maxSlides = parsePositiveInt(input.slidesMax, "--slides-max") ?? DEFAULT_MAX_SLIDES;
    const minDurationSeconds = parseNumberInRange(input.slidesMinDuration, "--slides-min-duration", {
        min: 0,
        max: 86_400,
