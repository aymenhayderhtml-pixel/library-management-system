...
export function resolveSlidesDir(outputDir, sourceId) {
    return path.join(outputDir, sourceId);
}
export function buildSlidesDirId(slidesDir) {
    return createHash("sha1").update(normalizePath(slidesDir)).digest("hex").slice(0, 8);
}
const isPathUnderRoot = (root, candidate) => {
    const rel = path.relative(root, candidate);
    return rel !== "" && !rel.startsWith("..") && !path.isAbsolute(rel);
};
export function resolveSlideImagePath(slidesDir, imagePath) {
    if (!imagePath)
        return null;
...
export function serializeSlideImagePath(slidesDir, imagePath) {
    const resolved = path.isAbsolute(imagePath) ? imagePath : path.join(slidesDir, imagePath);
    const normalizedSlidesDir = normalizePath(slidesDir);
    const normalizedResolved = normalizePath(resolved);
...
export async function validateSlidesCache({ cached, source, settings, }) {
    if (!cached || typeof cached !== "object")
        return null;
    if (cached.sourceId !== source.sourceId)
        return null;
    if (cached.sourceKind !== source.kind)
        return null;
    if (cached.sourceUrl !== source.url)
        return null;
    ...
    if (!isPathUnderRoot(normalizedOutputDir, normalizedExpectedDir))
        return null;
    ...
    if (cached.slidesDirId && cached.slidesDirId !== expectedDirId)
        return null;
    if (cached.sceneThreshold !== settings.sceneThreshold)
        return null;
    if (cached.maxSlides !== settings.maxSlides)
        return null;
    if (cached.minSlideDuration !== settings.minDurationSeconds)
        return null;
    if (cached.ocrRequested !== settings.ocr)
        return null;
...
export async function readSlidesCacheIfValid({ source, settings, }) {
