...
const JSON_RESULT_FIELDS = ["result", "response", "output", "message", "text"];
function isJsonCliProvider(provider) {
    return provider !== "codex";
}
function getCliProviderConfig(provider, config) {
    if (!config)
        return undefined;
    if (provider === "claude")
...
function extractJsonResultText(payload) {
...
function parseJsonProviderUsage(provider, payload) {
    if (provider === "claude")
...
function parseJsonProviderCostUsd(provider, payload) {
    if (provider !== "claude")
        return null;
    return toNumber(payload.total_cost_usd) ?? null;
}
function appendJsonProviderArgs({ provider, args, allowTools, model, prompt, }) {
