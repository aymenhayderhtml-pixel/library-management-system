...
export function resolvePreferredOnnxModel(env = process.env) {
...
export function isOnnxCliConfigured(model, env = process.env) {
    return resolveOnnxCommand(model, env) !== null;
}
const COMMAND_ENV_VAR = {
...
export function resolveOnnxProviderId(model) {
    return model === "parakeet" ? "onnx-parakeet" : "onnx-canary";
}
export function resolveOnnxCommand(model, env = process.env) {
    ...
    if (trimmed.startsWith("[")) {
        try {
            ...
            if (Array.isArray(parsed) &&
                ...
                return { kind: "argv", argvTemplate: parsed };
            }
        }
        catch {
...
function buildArgvCommand({ argvTemplate, inputPath, artifacts, }) {
...
function buildShellCommand({ commandTemplate, inputPath, artifacts, }) {
...
async function ensureModelArtifactsDownloaded({ model, notes, env, }) {
...
async function ensureWavInput({ filePath, mediaType, notes, }) {
    ...
    try {
        ...
        notes.push(`ONNX transcriber: ffmpeg transcode to WAV failed (${wrapError("ffmpeg", error).message}); using original input`);
        return { path: filePath, cleanup: null };
    }
}
export async function transcribeWithOnnxCli({ model, bytes, mediaType, filename, totalDurationSeconds = null, onProgress = null, env = process.env, }) {
...
export async function transcribeWithOnnxCliFile({ model, filePath, mediaType, totalDurationSeconds = null, onProgress = null, env = process.env, }) {
    const notes = [];
    ...
    if (!commandTemplate) {
        return {
            text: null,
            provider,
            error: new Error(`${provider}: command not configured (set ${COMMAND_ENV_VAR[model]} to a CLI that emits text from WAV audio)`),
            notes,
        };
    }
    let artifacts;
    ...
    const argvCommand = template.kind === "argv"
        ? buildArgvCommand({
        ...
        : null;
    const shellCommand = template.kind === "shell"
        ? buildShellCommand({
...
//# sourceMappingURL=onnx-cli.js.map