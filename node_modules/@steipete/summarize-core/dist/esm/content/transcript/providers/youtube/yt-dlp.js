...
import { probeMediaDurationSecondsWithFfprobe, transcribeMediaFileWithWhisper, } from "../../../../transcription/whisper.js";
...
const DEFAULT_AUDIO_FORMAT = "bestaudio[vcodec=none]/best[height<=360]/best[height<=480]/best[height<=720]/best";
export const fetchTranscriptWithYtDlp = async ({ ytDlpPath, transcription, env, groqApiKey, openaiApiKey, falApiKey, url, onProgress, service = "youtube", mediaKind = null, mediaCache = null, extraArgs, }) => {
    const notes = [];
    ...
    try {
        if (cachedMedia?.filePath) {
            ...
            await downloadAudio(ytDlpPath, url, outputFile, extraArgs, progress
...
export const fetchDurationSecondsWithYtDlp = async ({ ytDlpPath, url, }) => {
    if (!ytDlpPath)
        return null;
...
async function downloadAudio(ytDlpPath, url, outputFile, extraArgs, onProgress) {
    return new Promise((resolve, reject) => {
        ...
        const reportProgress = (downloadedBytes, totalBytes) => {
            ...
            if (normalizedTotal && normalizedTotal > 0) {
                const pct = Math.max(0, Math.min(100, Math.round((downloadedBytes / normalizedTotal) * 100)));
...
function emitProgressFromLine(line, onProgress) {
