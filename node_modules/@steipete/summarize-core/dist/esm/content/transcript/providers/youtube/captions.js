...
export function extractYoutubeDurationSeconds(html) {
...
function extractDurationSecondsFromPlayerPayload(payload) {
...
async function fetchYoutubePlayerPayload(fetchImpl, { html, videoId }) {
...
export async function fetchYoutubeDurationSecondsViaPlayer(fetchImpl, { html, videoId }) {
...
async function fetchTranscriptViaAndroidPlayer(fetchImpl, { html, videoId, skipAutoGenerated, }) {
...
export const fetchTranscriptFromCaptionTracks = async (fetchImpl, { html, originalUrl, videoId, skipAutoGenerated, }) => {
    const initialPlayerResponse = extractInitialPlayerResponse(html);
    if (initialPlayerResponse) {
        const transcript = await extractTranscriptFromPlayerPayload(fetchImpl, initialPlayerResponse, skipAutoGenerated);
        if (transcript) {
            return transcript;
        }
    }
    ...
    try {
        ...
        const transcript = await extractTranscriptFromPlayerPayload(fetchImpl, parsed, skipAutoGenerated);
        if (transcript) {
            return transcript;
        }
        return await fetchTranscriptViaAndroidPlayer(fetchImpl, { html, videoId, skipAutoGenerated });
...
const extractTranscriptFromPlayerPayload = async (fetchImpl, payload, skipAutoGenerated) => {
