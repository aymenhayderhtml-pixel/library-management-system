...
function isHiddenByStyle(style) {
    ...
    if (/clip-path\s*:\s*inset\(\s*100%/i.test(normalized))
        return true;
    if (/clip\s*:\s*rect\(\s*0(?:px)?\s*,\s*0(?:px)?\s*,\s*0(?:px)?\s*,\s*0(?:px)?\s*\)/i.test(normalized)) {
        return true;
    }
...
function shouldStripElement(tagName, style, attributes) {
    if (tagName === "template")
        return true;
    if (tagName === "script")
        return true;
...
export function stripHiddenHtml(html) {
    if (!html)
        return html;
