...
import { isAnubisHtml, isBlockedTwitterContent, isTwitterBroadcastUrl, isTwitterStatusUrl, toNitterUrls, } from "./twitter-utils.js";
...
const buildSkippedTwitterTranscript = (cacheMode, notes) => ({
    text: null,
    source: null,
    diagnostics: {
        cacheMode,
...
export async function fetchLinkContent(url, options, deps) {
    ...
    if (isTwitterBroadcastUrl(url)) {
        ...
        const transcriptDiagnostics = ensureTranscriptDiagnostics(transcriptResolution, cacheMode ?? "default");
        transcriptDiagnostics.notes = appendNote(transcriptDiagnostics.notes, "X broadcast: skipped HTML/Firecrawl");
        return finalizeExtractedLinkContent({
            url,
            baseContent: selectBaseContent("", transcriptResolution.text, transcriptResolution.segments),
            maxCharacters,
    ...
    if (isDirectMediaUrl(url) && mediaTranscriptMode === "prefer") {
        ...
        const transcriptDiagnostics = ensureTranscriptDiagnostics(transcriptResolution, cacheMode ?? "default");
        transcriptDiagnostics.notes = appendNote(transcriptDiagnostics.notes, "Direct media URL: skipped HTML/Firecrawl");
        return finalizeExtractedLinkContent({
            url,
            baseContent: selectBaseContent("", transcriptResolution.text, transcriptResolution.segments),
            maxCharacters,
    ...
    const attemptBird = async () => {
        ...
        try {
            ...
            const shouldAttemptTranscript = mediaTranscriptMode === "prefer" || (mediaTranscriptMode === "auto" && hasMedia);
            const autoModeNote = !shouldAttemptTranscript
                ...
                : null;
            const longFormNote = !hasMedia && text.length >= MAX_TWITTER_TEXT_FOR_TRANSCRIPT
            ...
            const result = finalizeExtractedLinkContent({
                url,
                baseContent: selectBaseContent(text, transcriptResolution.text, transcriptResolution.segments),
                maxCharacters,
                title,
                description,
                siteName,
                transcriptResolution,
                video: mediaUrl && media?.kind === "video"
                    ? {
                        kind: "direct",
                        url: mediaUrl,
                    }
